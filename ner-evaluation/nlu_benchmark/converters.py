...
tagger = spacy.load("en", parser=False)
    ...
    for index, row in data.iterrows():
        ...
        if str(row["answer_annotation"]) != "nan":
            ...
            generate_annotation_file(
                plain_sentence, scenario, intent, frame_elements, ann_id, output_dir
            )
        else:
            skipped += 1
    if verbose:
    ...
    if "[" in sentence:
        for annotation in annotations:
            ...
            frame_elements.append(
                {"frame_element": label, "filler": filler, "span": span}
            )
...
def generate_annotation_file(
    sentence, dialog_act, frame, frame_elements, ann_id, output_dir
):
    ...
    for i, token in enumerate(sentence_ann):
        ...
        if token.ent_type_:
            ...
            set_all_attrib_from_dict(
                ner_token_xml, {"id": str(i + 1), "value": ner_ann}
            )
        ...
        set_all_attrib_from_dict(
            frame_token_xml, {"id": str(i + 1), "value": frame_ann}
        )
    ...
    xmlstr = minidom.parseString(ET.tostring(example_xml)).toprettyxml(
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", type=str, default=None, help="Output dir")
